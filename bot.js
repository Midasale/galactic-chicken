...
const re = {
    ...
    thankyou: /^(?:\W*<@!?[\dA-F]+>)?\W*t(?:hank[ syoua]*| *y[ aou]*)(?:lot|(?:very )?much|ton|mil+(?:ion)|bunch)?(?: \w+){0,4}\W*(?:<@!?[\dA-F]+>\W*)?$/i,
				...
				if (err) {
					...
					if (data) {
						...
						result = rows
							...
							.sort((a,b) => b[1] - a[1]);
						const maxnumlen = result[0][1].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",").length;
